module.exports = (env) ->
  ...
  class HkAvrInputSelectActionHandler extends env.actions.ActionHandler
  ...
  class HkAvrInputSelectActionProvider extends env.actions.ActionProvider
    constructor: (@framework) ->
      super()

    parseAction: (input, context) =>
      selectorDevices = _(@framework.deviceManager.devices).values().filter(
        (device) => device.config.class is 'HkAvrInputSelector'
      ).value()
      ...
      if match?
        ...
        return {
          ...
          actionHandler: new HkAvrInputSelectActionHandler(@framework, device, valueTokens)
        }
      else
        return null

  return HkAvrInputSelectActionProvider
